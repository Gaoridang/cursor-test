...
env 없이도 **키워드 검색**까지 포함한 블로그는 바로 배포됩니다.  
xAI env를 나중에 넣으면 **시맨틱 / 전체** 탭이 자동으로 활성화됩니다.
...
- 홈 / 글 상세 페이지
- **키워드** 탭 BM25 검색 (`Cmd+K`, 한국어 기반)
- 시맨틱 / 전체 탭 → API 미설정 시 키워드 결과로 폴백
...
1. `build-search-index.mjs` → BM25 인덱스 생성 (한국어 토큰화)
...
키워드 검색은 한국어 어절 + 바이그램 색인을 사용합니다. 글을 추가·수정한 뒤에는 `npm run build` 또는 `node scripts/build-search-index.mjs`로 인덱스를 재생성하세요.
